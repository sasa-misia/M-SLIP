function [cn,on] = inpoly(p,node,edge,TOL,doFigures)
...
if doFigures
    figure
    plot(node(:,1),node(:,2),'+-')
    plot([node(edge(:,1),1) node(edge(:,2),1)]' ,[node(edge(:,1),2) node(edge(:,2),2)]','+-b')
    hold on
    plot(p(:,1),p(:,2),'ro')
end
...
tol = TOL*min(dxy);
%% [---------- Hilmar change
% there was a very anoying mistake here that made the tolerance equal to zero if testing with just one point
% where dxy=0. So I introduced a minimum tolerance
...
if doFigures
    if dxy(1)>dxy(2)
        % reverse previous flip
        p = p(:,[2,1]);
    end
    plot(p(cn,1),p(cn,2),'g*')
end
