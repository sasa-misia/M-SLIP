...
Choice = listdlg('PromptString',{'Select event analysed to plot:',''}, ...
...
switch LearnChoice
    case "Artificial Neural Network"
        Model = fitcnet(TrainingTable, TrainingFs);
%         TestError = loss(ModelNN,TrainingTable,TrainingFs,"LossFun","classiferror");

    case "Random Forest"
        Model = TreeBagger(50, TrainingTable, TrainingFs);
%         Model = TreeBagger(50,TrainingTable,TrainingFs,'Method','regression'); % If you want to use with numeric Fs
%         view(Model.Trees{1},'Mode','graph')

    case "Bag"
        Model = fitcensemble(TrainingTable, TrainingFs, 'Method','Bag', ...
                             'Learners',templateTree('Reproducible', true), ...
                             'Resample','on', 'NumLearningCycles',50);
%         Model = fitcensemble(TrainingTable, TrainingFs, 'Method','Bag', ...
%                              'Learners',templateTree('MaxNumSplits',TrainingSamples-1,'MinLeafSize',ConditioningFactors*2,'Reproducible',true), ...
%                              'Resample','on', 'FResample',0.8, 'NumLearningCycles',50);
%         Model = fitcensemble(TrainingTable, TrainingFs, 'Method','Bag', ...
%                              'Learners',templateTree('MaxNumSplits',TrainingSamples-1,'Reproducible',true), ...
%                              'Resample','on', 'FResample',0.8, 'NumLearningCycles',50);
%         Model = fitcensemble(TrainingTable, TrainingFs, 'Method','Bag', ...
%                              'Learners',templateTree('PredictorSelection','interaction-curvature','Surrogate','on','Reproducible',true), ...
%                              'Resample','on', 'FResample',0.8, 'NumLearningCycles',50);

    case "Adaptive Boosting"
        Model = fitcensemble(TrainingTable, TrainingFs, 'Method','AdaBoostM1', ...
                             'Learners',templateTree('Reproducible', true), ...
                             'LearnRate',0.1, 'NumLearningCycles',50);

    case "Adaptive logistic regression"
        Model = fitcensemble(TrainingTable, TrainingFs, 'Method','LogitBoost', ...
                             'Learners',templateTree('Reproducible', true), ...
                             'LearnRate',0.1, 'NumLearningCycles',50);
    ...
    case "Auto Machine Learning"
        Model = fitcensemble(TrainingTable, TrainingFs, 'OptimizeHyperparameters','auto');
%         Model = fitcensemble(TrainingTable, TrainingFs, 'OptimizeHyperparameters', ...
%                              {'NumLearningCycles','LearnRate','MaxNumSplits'});
%         Model = fitcensemble(TrainingTable, TrainingFs, 'OptimizeHyperparameters','auto', ...
%                              'Learners',templateTree('Reproducible',true), 'HyperparameterOptimizationOptions', ...
%                              struct('AcquisitionFunctionName','expected-improvement-plus'));
    case "Auto ANN"
        Model = fitcnet(TrainingTable,TrainingFs,'OptimizeHyperparameters','auto');
%         Model = fitcnet(TrainingTable,TrainingFs,'OptimizeHyperparameters','auto','Standardize',true);
end

[FsTrainingPrediction, FsTrainingScores] = predict(Model,TrainingTable);

if LearnChoice == "Random Forest"; [~, ~, FsTrainingCost] = predict(Model,TrainingTable); end

%% Prediction of all points in study area
AnalysisTableStudyArea = cell(1,size(AnalysisTable,1));
for i1 = 1:size(AnalysisTable,1)
    AnalysisTableStudyArea{i1} = array2table([AnalysisTable{i1,:}]);
