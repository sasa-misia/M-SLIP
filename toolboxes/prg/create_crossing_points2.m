function [ points ] = create_crossing_points2( points,n_pos,n_neg )
% This is an internal function to create crossing points of the PRG curve
n = n_pos+n_neg;
points.is_crossing = zeros(height(points),1);
% introduce a crossing point at the crossing through the y-axis
j = find(points.recall_gain>=0, 1, 'first');
...
if not(isempty(f))
    pr_ind = 1;
    crossings = array2table(zeros(numel(f'), size(points,2)), 'VariableNames',points.Properties.VariableNames);
    for i = f'
        ...
        new_point.is_crossing = 1;
        crossings(pr_ind, :)  = new_point;
        pr_ind = pr_ind + 1;
    end
    points = sortrows([points; crossings],[1,9]);
end
points.in_unit_square = ones(height(points),1);
