...
% h=mesh(X(ix),Y(ix),zeros(size(ix)),'marker',marker,'cdata',C(ix),'edgecolor','flat','markeredgecolor','flat','facecolor','flat',varargin{:});
tri = delaunay(X(ix), Y(ix));
h = trimesh(tri, X(ix), Y(ix), zeros(size(ix)), 'cdata', C(ix), 'edgecolor','none', 'facecolor','interp', varargin{:});
view(2)
