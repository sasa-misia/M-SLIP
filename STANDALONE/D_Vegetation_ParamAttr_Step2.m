...
elseif string(ChoiceVU) == string(Options{1})
    VUColors = zeros(size(SelectedSoil,1), 3);
...
if string(ChoiceDVC) == string(Options{2})
    DVCColors = cell2mat(cellfun(@(x) sscanf(x,'%d',[1 3]), ...
                 Sheet_DVCPar(2:size(Sheet_DVCPar,1),7), 'UniformOutput',false));
elseif string(ChoiceDVC) == string(Options{1})
    DVCColors = zeros(size(Sheet_DVCPar,1), 3);
...
VariablesVUPar = {'VU_DVCPlotColors', 'VUAbbr', 'DVCParameters'};
