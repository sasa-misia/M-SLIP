...
load([fold_res_ml_curr,sl,'ANNsMdlA.mat'], 'HistInfo','ANNs')
...
ANNsStrct = ANNs{'Structure',:};

ANNsNames = inputdlg2(strcat({'New name for '},ANNsNames), 'DefInp',ANNsNames);
ANNs.Properties.VariableNames = ANNsNames;
...
if SepMtrc
    CompMdl = true;
    Mdl2Plt = checkbox2(ANNsNames, 'Title',{'Models to plot:'});
end
...
if UnqMtrc
    Mdl     = 1:numel(ANNsNames); % It can be also an array but it must be horizontal (ex: Mdl = 1:10;)
...
MdlsNmbr  = numel(ANNsNames);

if RmvNanC
    ColsNotCompl = any(isnan(HistValLoss), 1); % [false, false, false];
    
    ANNsNames(:,ColsNotCompl)   = [];
    ANNsStrct(:,ColsNotCompl)   = [];
...
ANNsSens = table('RowNames',{'Val-Tst','Trn-Tst'});

ANNsSens{:,{'Raw','Smooth'}} = {table()};

ANNsSens{'Val-Tst','Raw'   } = {array2table([DiffValTstAUROC ; ...
                                             DiffValTstAUPRGC; ...
                                             DiffValTstLoss  ; ...
                                             DiffValTstMSE   ], 'RowNames',{'AUROC','AUPRGC','Loss','MSE'}, ...
                                                                'VariableNames',ANNsNames)};

ANNsSens{'Trn-Tst','Raw'   } = {array2table([DiffTrnTstAUROC ; ...
                                             DiffTrnTstAUPRGC; ...
                                             DiffTrnTstLoss  ; ...
                                             DiffTrnTstMSE   ], 'RowNames',{'AUROC','AUPRGC','Loss','MSE'}, ...
                                                                'VariableNames',ANNsNames)};

ANNsSens{'Val-Tst','Smooth'} = {array2table([SmDiffValTstAUROC ; ...
                                             SmDiffValTstAUPRGC; ...
                                             SmDiffValTstLoss  ; ...
                                             SmDiffValTstMSE   ], 'RowNames',{'AUROC','AUPRGC','Loss','MSE'}, ...
                                                                  'VariableNames',ANNsNames)};

ANNsSens{'Trn-Tst','Smooth'} = {array2table([SmDiffTrnTstAUROC ; ...
                                             SmDiffTrnTstAUPRGC; ...
                                             SmDiffTrnTstLoss  ; ...
...
ANNsHist = table('RowNames',DsetPrt);

ANNsHist{:,{'Raw','Smooth'}} = {table()};

ANNsHist{'Trn','Raw'   } = { table({array2table(HistTrnAUROC , 'RowNames',IterNames, 'VariableNames',ANNsNames)}, ...
                                   {array2table(HistTrnAUPRGC, 'RowNames',IterNames, 'VariableNames',ANNsNames)}, ...
                                   {array2table(HistTrnLoss  , 'RowNames',IterNames, 'VariableNames',ANNsNames)}, ...
                                   {array2table(HistTrnMSE   , 'RowNames',IterNames, 'VariableNames',ANNsNames)}, 'VariableNames',{'AUROC','AUPRGC','Loss','MSE'}, ...
                                                                                                                  'RowNames',{'Hist'}) };

ANNsHist{'Val','Raw'   } = { table({array2table(HistValAUROC , 'RowNames',IterNames, 'VariableNames',ANNsNames)}, ...
                                   {array2table(HistValAUPRGC, 'RowNames',IterNames, 'VariableNames',ANNsNames)}, ...
                                   {array2table(HistValLoss  , 'RowNames',IterNames, 'VariableNames',ANNsNames)}, ...
                                   {array2table(HistValMSE   , 'RowNames',IterNames, 'VariableNames',ANNsNames)}, 'VariableNames',{'AUROC','AUPRGC','Loss','MSE'}, ...
                                                                                                                  'RowNames',{'Hist'}) };

ANNsHist{'Tst','Raw'   } = { table({array2table(HistTstAUROC , 'RowNames',IterNames, 'VariableNames',ANNsNames)}, ...
                                   {array2table(HistTstAUPRGC, 'RowNames',IterNames, 'VariableNames',ANNsNames)}, ...
                                   {array2table(HistTstLoss  , 'RowNames',IterNames, 'VariableNames',ANNsNames)}, ...
                                   {array2table(HistTstMSE   , 'RowNames',IterNames, 'VariableNames',ANNsNames)}, 'VariableNames',{'AUROC','AUPRGC','Loss','MSE'}, ...
                                                                                                                  'RowNames',{'Hist'}) };

ANNsHist{'Trn','Smooth'} = { table({array2table(SmHistTrnAUROC , 'RowNames',IterNames, 'VariableNames',ANNsNames)}, ...
                                   {array2table(SmHistTrnAUPRGC, 'RowNames',IterNames, 'VariableNames',ANNsNames)}, ...
                                   {array2table(SmHistTrnLoss  , 'RowNames',IterNames, 'VariableNames',ANNsNames)}, ...
                                   {array2table(SmHistTrnMSE   , 'RowNames',IterNames, 'VariableNames',ANNsNames)}, 'VariableNames',{'AUROC','AUPRGC','Loss','MSE'}, ...
                                                                                                                    'RowNames',{'Hist'}) };

ANNsHist{'Val','Smooth'} = { table({array2table(SmHistValAUROC , 'RowNames',IterNames, 'VariableNames',ANNsNames)}, ...
                                   {array2table(SmHistValAUPRGC, 'RowNames',IterNames, 'VariableNames',ANNsNames)}, ...
                                   {array2table(SmHistValLoss  , 'RowNames',IterNames, 'VariableNames',ANNsNames)}, ...
                                   {array2table(SmHistValMSE   , 'RowNames',IterNames, 'VariableNames',ANNsNames)}, 'VariableNames',{'AUROC','AUPRGC','Loss','MSE'}, ...
                                                                                                                    'RowNames',{'Hist'}) };

ANNsHist{'Tst','Smooth'} = { table({array2table(SmHistTstAUROC , 'RowNames',IterNames, 'VariableNames',ANNsNames)}, ...
                                   {array2table(SmHistTstAUPRGC, 'RowNames',IterNames, 'VariableNames',ANNsNames)}, ...
                                   {array2table(SmHistTstLoss  , 'RowNames',IterNames, 'VariableNames',ANNsNames)}, ...
                                   {array2table(SmHistTstMSE   , 'RowNames',IterNames, 'VariableNames',ANNsNames)}, 'VariableNames',{'AUROC','AUPRGC','Loss','MSE'}, ...
                                                                                                                    'RowNames',{'Hist'}) };
...
if SepMtrc
    MdlStrc = ANNsStrct(ismember(Mdl2Plt, ANNsNames));
    if CompMdl
        if numel(Mdl2Plt) > 3
    ...
    TbIdAss = listdlg2(strcat("Row name for ",DsetUsd), ANNsHist.Properties.RowNames);
    ...
    if CompMdl
        ClrsIds = cellfun(@(x) Colors(x(2,:)), IndComb, 'UniformOutput',false);
        LnType  = cellfun(@(x) LnOpts(x(1,:)), IndComb, 'UniformOutput',false);
    else
        ClrsIds = cellfun(@(x) Colors(x(1,:)), IndComb, 'UniformOutput',false);
    ...
    for i1 = 1:numel(Mdl2Plt)
        for i2 = 1:numel(FigCont)
            ...
            for i3 = 1:numel(GenLbls{i1})
                hLnPlt{i3} = plot(1:HistNmbr, ANNsHist{TblIdsR{i1}(i3), ...
...
if DffMtrc    
    ...
    for i1 = 1:numel(GenCapt)
        for i2 = 1:numel(FigCont)
            ...
            AvVal = mean(ANNsSens{GenCapt{i1},CrvType}{:}{FigCont{i2},:});
    
            hLin1 = plot(1:MdlsNmbr, ANNsSens{GenCapt{i1},CrvType}{:}{FigCont{i2},:}, 'Color',ClrCont{i2}, 'LineWidth',LnWidth, 'Parent',hAxsDiff);
...
if UnqMtrc
    ...
    for i1 = Mdl
        ...
        SubTxt = ['ANN structure: [',strjoin({num2str(ANNsStrct{i1})}),']'];
        ...
        for i2 = 1:numel(GenLbls{1})
            hLnLft{i2} = plot(1:HistNmbr, ANNsHist{TblIdsR{1}(i2), ...
        ...
        for i2 = 1:numel(GenLbls{2})
            hLnRgt{i2} = plot(1:HistNmbr, ANNsHist{TblIdsR{2}(i2), ...
