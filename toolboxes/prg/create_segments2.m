function [ segments ] = create_segments2(labels,pos_scores,neg_scores)
...
j = 0;
counts = nan(length(t.labels), 2);
[new_pos_scores, new_neg_scores] = deal(nan(1, length(t.labels)));
...
new_pos_scores(isnan(new_pos_scores)) = [];
new_neg_scores(isnan(new_neg_scores)) = [];
